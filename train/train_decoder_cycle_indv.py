...
from train.cycle_utils import get_homogeneous_speaker_pair
from train.frozen_assets import FrozenAssets
...
if __name__ == "__main__":
    ...
    for spk in speakers :
        log_dir = f'log/Decoder_cycle_only_{coef_cyc}/local/{codebooksize}_diff_{learning_rate}/{spk}'
        ...
        frozen = FrozenAssets(codebooksize, params.embedding_dim, device)
        quantizer = frozen.quantizer(spk, params.commitment_cost)
        ...
        for epoch in range(epochs):
            ...
            for batch in pbar:
                ...
                src_spk, tgt_spk = get_homogeneous_speaker_pair(batch, context="train")
                ...
                # Map & Quantizer Load
                quantizer_tgt = frozen.quantizer(tgt_spk)

                # Mapping A -> B
                final_map_A_to_B = frozen.argmax(src_spk, tgt_spk)
                ...
                # Map & Quantizer Load
                quantizer_src = frozen.quantizer(src_spk)

                # Mapping B -> A
                final_map_B_to_A = frozen.argmax(tgt_spk, src_spk)
            ...
            with torch.no_grad():
                for batch in tqdm(val_loader, desc=f"Ep {epoch+1}/{epochs} [Valid]"):
                    ...
                    src_spk, tgt_spk = get_homogeneous_speaker_pair(batch, context="valid")

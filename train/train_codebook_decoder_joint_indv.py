...
import params
from train.prepare_data_indv import VCDecBatchCollate, VCTKDecDataset_indv
...
if __name__ == "__main__":
    ...
    for spk in speakers :
        log_dir = f'log/Codebook_decoder_joint/local/{codebooksize}_diff_{learning_rate}_code_{code_lr}/{spk}'

        os.makedirs(log_dir, exist_ok=True)
        print(log_dir)
        msg = f"""
        Training Quantizer Decoder Joint
        ...
        train_set = VCTKDecDataset_indv(data_dir_train, exc_file, mode = 'train', spk = spk)
        val_set = VCTKDecDataset_indv(data_dir_valid, exc_file, mode = 'valid', spk = spk)

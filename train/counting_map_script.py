...
for (spk1, spk2) in permutations(speakers, 2):
    ...
    with torch.no_grad():
        for batch in tqdm(train_loader, leave=False):
            ...
            # Count all co-occurrences in one device operation.
            linear_indices = indices_src.long() * codebooksize + indices_tgt.long()
            counts = torch.bincount(linear_indices, minlength=codebooksize ** 2)
            count_indv2indv += counts.reshape(codebooksize, codebooksize)

...
def inference(src_speaker, tgt_speaker, src_wav_path, tgt_wav_path, output_path, codebooksize, 
    ...
    if decoder_base_path:
        ...
        # Fallback
        ckpt_dec = torch.load("log/log_Gunhee/vc_255.pt", map_location=device)
    ...
    with torch.no_grad():
        ...
        # A의 Codebook에서 인덱스 추출
        indices_src = quantizer_src.get_code_indices(mean) # (B, T)

        # 최종 맵을 참조하여 B의 인덱스로 변환
        indices_tgt = final_map_A_to_B[indices_src.flatten()] # (B*T)

        # B의 코드북에서 해당 인덱스의 벡터들을 가져옴
        target_codewords = quantizer_tgt.embeddings.weight
        final_vectors = target_codewords[indices_tgt] # (B*T, D)
        ...
        # forward_vq 수행
        mel_encoded_vq, mel_vq = generator.forward_vq(
            mel_source, mel_source_lengths, mel_target, mel_target_lengths, embed_target,
            mean_quantized, mean_ref_quantized, n_timesteps=30, mode='ml'
        )
        
        mel_synth_np_vq = mel_vq.cpu().detach().squeeze().numpy()
        mel_source_np_vq = mel_source.cpu().detach().squeeze().numpy() 
        
        mel_vq = torch.from_numpy(
            mel_spectral_subtraction(mel_synth_np_vq, mel_source_np_vq, smoothing_window=1)
        ).float().unsqueeze(0)
        
        if use_gpu: 
            mel_vq = mel_vq.cuda()

    # 5. 오디오 생성 및 저장 (HiFi-GAN)
    with torch.no_grad():
        audio_vq = hifigan_universal.forward(mel_vq).cpu().squeeze().clamp(-1, 1)
        print(audio_vq.shape)
        
    sr = 22050
    sf.write(f'{output_path}', audio_vq.numpy(), sr) 
